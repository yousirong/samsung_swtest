/*
    ...
    ■ 자료 구조 : 칸마다 스택, 말마다 자기 높이
      ...
      그래서 칸을 배열 스택으로 관리하고, 말은 자기가 몇 번째 층인지 기억한다.
        ...
        board2[r][c][i] : (r, c)에 쌓인 아래에서 i번째 말의 번호
...
int board2[MAX][MAX][10 + 3];  // board2[r][c][i] : 아래에서 i번째 말의 번호
...
void input(const std::vector<std::vector<int>>& board, const std::vector<std::vector<int>>& horses)
{
    ...
    for (int k = 0; k < K; k++)
    {
        ...
        r = horses[k][0];     // [수정] scanf 대체
        c = horses[k][1];     // [수정] scanf 대체
        dir = horses[k][2];   // [수정] scanf 대체
        ...
        board2[r][c][position] = hcnt;  // 칸은 말 번호를 기억하고
...
void moveWhite(int number)
{
    ...
    {
        int selectedNumber = board2[sr][sc][k];
        ...
        board2[nr][nc][nextBack++] = selectedNumber;
    }
