...
void input(const std::vector<std::vector<int>>& board, const std::vector<std::vector<int>>& queries)
{
    ...
    Q = (int)queries.size();      // [수정] scanf 대체
    ...
    for (int r = 1; r <= N; r++)
        for (int c = 1; c <= M; c++)
            circle[r][c] = board[r - 1][c - 1];   // [수정] scanf 대체

    for (int i = 1; i <= Q; i++)
    {
        X[i] = queries[i - 1][0];   // [수정] scanf 대체 (원본이 1-based로 쓰므로 인덱스를 맞춘다)
        D[i] = queries[i - 1][1];   // [수정] scanf 대체
        K[i] = queries[i - 1][2];   // [수정] scanf 대체
    }
}
