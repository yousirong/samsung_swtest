...
void input(const std::vector<std::vector<int>>& blocks)
{
    ...
    K = (int)blocks.size();   // [수정] scanf 대체

    for (int k = 0; k < K; k++)
    {
        TYPE[k] = blocks[k][0];   // [수정] scanf 대체
        R[k] = blocks[k][1];      // [수정] scanf 대체
        C[k] = blocks[k][2];      // [수정] scanf 대체
    }
}
...
void simulate()
{
    ...
    ansScore = score;  ansBlockCount = blockCount;   // [수정] printf -> 전역에 저장
}
