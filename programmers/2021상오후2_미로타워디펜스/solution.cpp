/*
	...
	■ 원본(백준 21611)과 다른 점 두 가지
	  타워 디펜스로 각색되면서 규칙이 바뀐 부분이 있어, 원본 풀이를 그대로 가져오면 틀린다.

	  1) 방향 번호가 0부터 시작한다
	     dr / dc가 4칸짜리이고 d를 그대로 인덱스로 쓴다. 즉 0=→, 1=↓, 2=←, 3=↑ 다.
	     (백준 21611은 1~4로 1=위, 2=아래, 3=왼쪽, 4=오른쪽이다)
	     같은 폴더의 예제 입력 input.txt의 마지막 줄이 "0 2"인 것에서도 0부터임을 알 수 있다.
	     대신 배열이 4칸뿐이라 d가 4 이상으로 들어오면 배열 밖을 읽게 되니,
	     방향이 4개를 넘는 변형 문제에 이 코드를 옮길 때는 배열 크기를 먼저 손봐야 한다.

	  2) 공격으로 처치한 몬스터도 점수가 된다
	     simulate에서 score += MAP[nr][nc] 로 공격에 맞아 사라진 몬스터의 번호를 점수에 더한다.
	     백준 21611에서는 마법으로 파괴된 구슬에 점수가 없고 폭발만 점수를 주지만,
	     이 문제에서는 처치 점수가 있다.
	     예제(input.txt)의 정답이 29인데, 이 줄을 빼면 25가 나오는 것으로 확인된다.
...
// 타워가 공격하는 방향. 입력 d를 그대로 인덱스로 쓴다 (0부터 시작, 백준 21611과 다름)
// →, ↓, ←, ↑
int dr[] = { 0, 1, 0, -1 };
int dc[] = { 1, 0, -1, 0 };
...
void input(const std::vector<std::vector<int>>& board, const std::vector<std::vector<int>>& commands)
{
	...
	for (int m = 0; m < M; m++)
	{
		D[m] = commands[m][0];   // [수정] scanf 대체
		P[m] = commands[m][1];   // [수정] scanf 대체
	}
}
...
int simulate()
{
	...
	for (int m = 0; m < M; m++)
	{
		...
		for (int k = 1; k <= p; k++)
		{
			...
			// 처치한 몬스터의 번호가 그대로 점수가 된다.
			// 원본(BOJ 21611)에는 없는 규칙이니 주의 (파일 상단 "원본과 다른 점" 참고)
			score += MAP[nr][nc];
			MAP[nr][nc] = 0;
